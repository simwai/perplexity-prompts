...
export interface DecisiveResult {
  ...
  verdict: "BOUNDARY" | "NO-BOUNDARY";
  rationale: string;
}

export type AblationName = "global-trust" | "noisy-07-08" | "noisy-05-05" | "windowed-history" | "no-snapshot";

export const ABLATION_NAMES: AblationName[] = [
  "global-trust",
  "noisy-07-08",
  "noisy-05-05",
  "windowed-history",
...
export function keyTaskType(key: number): string {
...
function flipsKey(key: number, flipped: ReadonlyArray<string>): boolean {
...
function predict(policy: PolicyName, belief: KeyBelief): number {
  if (policy === "vanilla") return belief.first ?? 1;
  if (policy === "decay") return belief.ema >= 0.5 ? 1 : 0;
  return belief.support1 >= belief.support0 ? 1 : 0;
}

function predictedMw(policy: PolicyName, belief: KeyBelief): number {
  if (policy === "vanilla") return belief.first === null ? 0.5 : belief.first;
  if (policy === "decay") return belief.ema;
...
function observe(
  ...
  switch (policy) {
    case "vanilla":
      if (belief.first === null) belief.first = truth;
      return;
    case "no-forgetting":
      if (truth === 1) belief.support1 += 1;
  ...
  for (let ep = 0; ep < EPISODES; ep++) {
    ...
    if (regime === "B" && ep >= SHIFT_AT) {
      ...
      if (ttr === null && window.length === TTR_WINDOW) {
        ...
        if (correct / TTR_WINDOW >= TTR_TARGET) ttr = ep - SHIFT_AT + 1;
      }
    }
  }
...
function windowedRun(seed: number): PolicyRun {
  const queries = createSeededRandom(seed);
...
function globalRun(seed: number): PolicyRun {
  const queries = createSeededRandom(seed);
  const flipped = flippedTypes(seed);
  const truth: number[] = [];
  for (let k = 0; k < KEY_COUNT; k++) {
...
export function runAblation(name: AblationName, seed: number, lambda: number): AblationReport {
  switch (name) {
    case "global-trust": {
...
export function selectDecayLambda(seed: number): number {
...
function findReport(reports: PolicyReport[], policy: PolicyName): PolicyReport {
  for (const report of reports) {
    if (report.policy === policy) return report;
  }
  throw new Error(`missing report for ${policy}`);
}

export function runDecisive(seed: number): DecisiveResult {
