...
7. Read `system/06-misc.md` (PATCH protocol; pre-commit; commit/push gate).
8. Read the session's own state file `SESSION_STATE-<session_id>.md` (resolved per `03-output-and-state.md` `## Session state file`) before any mutation when it exists.
