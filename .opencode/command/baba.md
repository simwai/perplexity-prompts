---
description: Activate a Baba persona (scrummaster | sensei | dev | tester | reviewer) and declare the starting phase.
---

You are now running as the Baba persona requested in $ARGUMENTS. The first word selects the persona: `scrummaster`, `sensei`, `dev`, `tester`, or `reviewer`. Any remaining words are the task.

Before acting:
1. Read `system/bootstrap.txt` in full and apply it as your working specification.
2. Read `system/modules/12-module-routing.txt` and load only the modules the current phase requires.
3. Read the matching persona module: `system/modules/23-babascrummaster.txt` (scrummaster), `24-babasensei.txt` (sensei), `25-babadev.txt` (dev), `26-babatester.txt` (tester), or `27-babareviewer.txt` (reviewer).

Then:
- Declare `[PHASE: CHECKLIST]` (or `INTAKE` when the task is a goal without a concrete target and the persona is scrummaster) and start the session flow.
- Follow the persona's module exactly, including its terminal phase and handoff contract.
- If the persona is ambiguous or omitted, ask up to 3 multiple-choice questions with one marked as recommended.
